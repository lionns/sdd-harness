...
Every released version of the SDD harness, newest first. The rules that govern a version — what
`MAJOR`, `MINOR` and `PATCH` mean, and what a change to the harness requires — live in
[`docs/sdd/VERSION.md`](docs/sdd/VERSION.md).

### 0.9.0 — 2026-09-03

The release history leaves the rules budget, and an installed harness can prove it is one.
D-032, D-033.

- **`CHANGELOG.md`.** The history moves to the repository root and `docs/sdd/VERSION.md` keeps the
  rules. `knownVersions` reads the new path and falls back to the old one, so a repository upgrades
  before moving its own history, or without ever moving it.
- **`harness.lock`.** `harness-init` installs a hash manifest of the vendored surface, and
  `harness-lint` verifies it wherever it finds one: a drifted copy fails, naming the file and the
  repository the change belongs in. No lock, no check. Regenerate it with
  `node scripts/harness-manifest.mjs` in the commit that changes governance.

### 0.8.1 — 2026-09-03

Budget keys now fail when missing or unenforced. To migrate from 0.7.1, replace `taskFileLines`
with `taskPlanLines` and `taskRecordLines`, and remove `journalEntryLines`. D-031.

### 0.8.0 — 2026-09-03

Task plans and execution records have separate limits. D-030.
...
- **Split task budgets.** `taskPlanLines` covers everything before the first `## Outcome`, while
  ...
  and a heading inside a fenced code block is not one.
